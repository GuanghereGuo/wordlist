...
\documentclass{mancls} % 调用 mancls 文档类 (不加 code 选项即为说明书)
...
\subsection{软件概述}
本系统是一款集成了数据采集、自然语言处理与文档自动化管理的桌面端软件。系统采用 Python 语言开发，深度整合了 Excel 文档处理技术与网络爬虫技术。其核心功能是读取用户指定的 Excel 单词表，自动连接在线词典接口（如 Youdao API），精准抓取单词的音标（Phonetic）、释义（Definition）及权威例句（Example），并将数据智能回填至原表格中。
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.8\textwidth]{config_code.png}
%     \caption{配置文件参数详情}
%     \label{fig:config}
% \end{figure}
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.9\textwidth]{excel_raw.png}
...
\begin{tikzpicture}[node distance=1.5cm]
    ...
    \node (config) [io, below of=start] {读取 config.yaml};
    \node (backup) [process, below of=config] {执行自动备份};
    \node (loop) [decision, below of=backup, yshift=-0.5cm] {遍历\newline 行数据};
    \node (check) [decision, below of=loop, yshift=-1.5cm] {检查\newline 状态};
    \node (fetch) [process, right of=check, xshift=3cm] {网络抓取数据};
    \node (save) [process, below of=fetch] {写入Excel单元格};
    \node (end) [startstop, left of=save, xshift=-3cm] {保存并结束};

    \draw [arrow] (start) -- (config);
    \draw [arrow] (config) -- (backup);
    \draw [arrow] (backup) -- (loop);
    \draw [arrow] (loop) -- node[anchor=east] {有数据} (check);
    \draw [arrow] (loop) -| node[anchor=south] {无更多数据} (end);
    \draw [arrow] (check) -- node[anchor=south] {未处理} (fetch);
    \draw [arrow] (check) -- node[anchor=west] {已处理/失败} (loop); % 回路
    \draw [arrow] (fetch) -- (save);
    \draw [arrow] (save) |- (loop);
\end{tikzpicture}
\caption{软件核心处理逻辑流程图}
\end{figure}
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.8\textwidth]{backup_folder.png}
